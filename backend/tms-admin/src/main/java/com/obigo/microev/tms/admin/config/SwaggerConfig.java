...
public class SwaggerConfig {
    ...
    public OpenAPI openAPI() {
        ...
        return new OpenAPI()
                ...
                .addServersItem(new Server().url("http://localhost:8090" + contextPath).description("Local Server"))
    ...
    private RequestBody createLoginRequestBody() {
        ...
        properties.put("email", new Schema<>().type("string").example("admin@obigo.com").description("Email(ID)"));
        properties.put("password", new Schema<>().type("string").example("1234").description("비밀번호").nullable(false));

        Schema<?> schema = new Schema<>().type("object").required(List.of("email", "password")).properties(properties);
